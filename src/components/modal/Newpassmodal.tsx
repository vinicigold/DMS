...
import { LockClosedIcon, XMarkIcon, EyeIcon, EyeSlashIcon, ExclamationTriangleIcon, CheckCircleIcon } from '@heroicons/react/24/solid'
...
export default function NewPassModal({
  ...
  const passwordStrength = password.newPassword.length > 0 ? Math.max(0,5 - errors.length): 0
  const strengthColors = ['bg-red-500', 'bg-orange-500', 'bg-yellow-500', 'bg-blue-500', 'bg-green-500']
  const textStrengthColors = ['text-red-600', 'text-yellow-600', 'text-green-600']
    ...
    <div className='fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50'>
      <div className='bg-white rounded-3xl shadow-3xl w-full max-w-sm transform transition-all duration-300 scale-100'>
        ...
        <form onSubmit={handleSubmit} className='p-6'>
          <div className='mb-4'>
            <label htmlFor='newPassword' className='block text-[#112D4E] font-semibold mb-3 text-sm'>New Password</label>
            ...
            {password.newPassword && (
              <div className='mt-3'>
                ...
                <p className={`text-xs font-medium ${passwordStrength > 0 ? textStrengthColors [passwordStrength - 1]: 'text-red-600'}`}>
                  {passwordStrength > 0 
          ...
          <div className='mb-6'>
            <label htmlFor='confirmPassword' className='block text-[#112D4E] font-semibold mb-3 text-sm'>Confirm Password</label>
            <div className='relative'>
              <LockClosedIcon className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
              <input
                ...
                onChange={(e) => handleNewPassChange('confirmPassword', e.target.value)}
              ...
              <button
                ...
                className='absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-[#3F72AF] transition-colors'>
                  {showPassword.confirmPassword ? <EyeSlashIcon className="w-5 h-5" />
            ...
            <div className='mb-6 p-4 bg-gray-50 rounded-xl'>
              ...
              <ul className='space-y-1'>
                {[
                  ...
                  <li key={req} className={`text-sx flex items-center gap-2 
                    ${!errors.includes(req) ? 'text-green-600'
                    :'text-gray-500'}`}>
                      <CheckCircleIcon className={`w-3 h-3 ${!errors.includes(req) ? 'text-green-600'
                        :'text-gray-400'}`} />
                      {req}
                  </li>
                ))}
          ...
          <button
            type='submit'
            disabled={isLoading || errors.length > 0 || password.newPassword !== password.confirmPassword || !password.newPassword}
