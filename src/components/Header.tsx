...
export default function Head() {
  ...
  return (
    <header className="h-16 w-full bg-[#1C3879] flex items-center shadow justify-between px-6 ">
      <Image src="/dmsbgcut.png" alt="Logo" width={150} height={100} className=" r-2" />
      <div className="flex items-center text-sm text-white gap-2 relative">
        <span className="">Welcome, User</span>
        <button onClick={() => setDropDown(!dropDown)} className="flex items-center gap-2 relative">
          {dropDown ? (
        ...
        {dropDown && (
          <div className="absolute right-2 top-11 bg-[#112D4E] shadow-lg rounded-lg p-4">
            <button onClick={handleLogout} className="text-[#ffffff] hover:text-[#3F72AF]">Logout</button>
          </div>
        ) }
      </div>
    </header>
